...
columns_names_to_drop = ['weather', 'weather_icon', 'sys_type']
...
def normalize_weather_columns(df: pd.DataFrame) -> pd.DataFrame:
     ...
     logging.info(f"-> Colunas de weather normalizadas - {len(df.columns)} colunas")
     return df

def drop_coluns(df: pd.DataFrame, colums_names:list[str]) -> pd.DataFrame:
...
def data_transformations():
    ...
    df = drop_coluns(df, columns_names_to_drop)
    df = rename_columns(df, columns_names_to_rename)
    df = normalize_datetime_columns(df, columns_to_normalize_datetime)
    logging.info(f"-> Processo de transformação dos dados concluído com sucesso!")
    return df